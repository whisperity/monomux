...
/// of \p Poll.
template <typename T>
static std::size_t sendMessageAndRescheduleIfOverflow(
  system::IOEvent& Poll, system::BufferedChannel& Channel, const T& Msg)
{
  try
  {
    return message::sendMessage(Channel, Msg);
  }
...
void Server::sendAcceptClient(ClientData& Client)
{
  sendMessageAndRescheduleIfOverflow(
    *Poll,
    ...
    message::notification::Connection{{true}, {}});
}
...
HANDLER(requestMakeSession)
{
...

  {
    ...
    MS.Socket = Platform::SocketPath::absolutise(Server.Sock->identifier());
  ...
  std::unique_ptr<Process> P = Process::spawn(SOpts);

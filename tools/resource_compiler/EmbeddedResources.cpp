...
std::optional<std::string_view> get(std::string_view Identifier) noexcept
{
  ...
  return std::string_view{AR.begin, AR.size};
}
  ...
  EMBEDDED_RESOURCES_GETTER(const char* Identifier)
{
  ...
  return EMBEDDED_RESOURCES_ARRAY_REF{Res->begin(), Res->end(), Res->size()};
...
#define RESOURCE_BUFFER(SYMBOL_NAME, SIZE)                                     \
  constexpr const char SYMBOL_NAME[SIZE]
#define RESOURCE_INIT(DEVELOPER_NAME, SYMBOL_NAME, SIZE)                       \
  StaticPtr->Map[DEVELOPER_NAME] = ArrayPtr{SYMBOL_NAME, SIZE};
